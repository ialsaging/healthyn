---
title: 'Forest Plot of interaction term (controlling for demographics)'
output: 
  html_document:
    toc: TRUE
    toc_depth: 2
...
This document contains the forest plot summarizing the cov1-sectional analyses described in the manuscript. 
...
```{r load_data, echo = F}
load(here("behavior/created data/meta_smoker_cov1.Rdata"))
load(here("behavior/created data/meta_drinker_cov1.Rdata"))
load(here("behavior/created data/meta_active_cov1.Rdata"))
```


```{r merge, warning = F, message = F, echo = F}
smoker.cov1.data <- smoker.cov1.data %>%
  mutate(outcome = "smoker")

drinker.cov1.data <- drinker.cov1.data %>%
  mutate(outcome = "drinker")

active.cov1.data <- active.cov1.data %>%
  mutate(outcome = "active")

data_cov1_all <- smoker.cov1.data %>%
  full_join(drinker.cov1.data) %>%
  full_join(active.cov1.data) %>%
...
max.ci.cov1 = max(exp(data_cov1_all$estimate + 1.96*data_cov1_all$std.error))
  # find minimum lower bound of confidence intervals
min.ci.cov1 = min(exp(data_cov1_all$estimate - 1.96*data_cov1_all$std.error))

range.cov1 = max.ci.cov1-min.ci.cov1

#give some space on either side. These are the plot bounds
lower.cov1 = min.ci.cov1-(range.cov1) 
upper.cov1 = max.ci.cov1+(range.cov1)

#estimate position of extra information
lower.cov1 = min.ci.cov1-(range.cov1)
upper.cov1 = max.ci.cov1+(range.cov1*.75)

#estimate position of extra information
  # range of space between left side of plot and lowerbound of confidence intervals
pos.cov1 = min.ci.cov1-lower.cov1
  # break that into equal sections. 
pos.cov1 = pos.cov1/4
  #use the sections for figure out where you want each column to be in that space
pos.cov1 = c(lower.cov1+3*pos.cov1)
```
...
total.active = nrow(active.cov1.data)
rows.active = c(1, total.active)

# which rows are for drinker
total.drinker = nrow(drinker.cov1.data)
...
total.smoker = nrow(smoker.cov1.data)
...
forest(data_cov1_all$estimate, #estimate
       data_cov1_all$std.error^2, #variance
       xlim = c(lower.cov1, upper.cov1), #limits of x-axis, full figure
       ylim = c(-1, nrow(data_cov1_all) + extra.space*2 + 2), # limits of y-axis
       cex = cex.value, #font size
       slab = data_cov1_all$study, #study label
       ...
       ilab = data_cov1_all[,c("n")],
       # where does extra information go?
       ilab.xpos = pos.cov1)

# add weighted average effects polygon
addpoly(smoker.cov1.rma, row=rows.smoker[1]-1, cex = cex.value, transf = exp, mlab = "", col = "blue")
addpoly(drinker.cov1.rma, row=rows.drinker[1]-1, cex = cex.value, transf = exp, mlab = "", col = "blue")
addpoly(active.cov1.rma, row=rows.active[1]-1, cex = cex.value, transf = exp, mlab = "", col = "blue")

### add text with Q-value, dfs, p-value, and I^2 statistic for subgroups
text(x = lower.cov1, y = rows.smoker[1]-2.25, pos=4, cex = cex.value,
     bquote(paste("RE Model for smoking status analyses (Q = ",
                  .(formatC(smoker.cov1.rma$QE, digits=2, format="f")),
                  ", df = ", .(smoker.cov1.rma$k - smoker.cov1.rma$p),
                  ", p = ", .(formatC(smoker.cov1.rma$QEp, digits=2, format="f")), "; ", I^2, " = ",
                  .(formatC(smoker.cov1.rma$I2, digits=1, format="f")), "%)")))
text(x = lower.cov1, y = rows.drinker[1]-2.25, pos=4, cex = cex.value,
     bquote(paste("RE Model for alcoholic consumption analyses (Q = ",
                  .(formatC(drinker.cov1.rma$QE, digits=2, format="f")),
                  ", df = ", .(drinker.cov1.rma$k - drinker.cov1.rma$p),
                  ", p = ", .(formatC(drinker.cov1.rma$QEp, digits=2, format="f")), "; ", I^2, " = ",
                  .(formatC(drinker.cov1.rma$I2, digits=1, format="f")), "%)")))
text(x = lower.cov1, y = rows.active[1]-2.25, pos=4, cex = cex.value,
     bquote(paste("RE Model for physical activity analyses (Q = ",
                  .(formatC(active.cov1.rma$QE, digits=2, format="f")),
                  ", df = ", .(active.cov1.rma$k - active.cov1.rma$p),
                  ", p = ", .(formatC(active.cov1.rma$QEp, digits=2, format="f")), "; ", I^2, " = ",
                  .(formatC(active.cov1.rma$I2, digits=1, format="f")), "%)")))

#bold font
par(font = 2)

# add "overall"" label to each set of analyses
text(lower.cov1, y = c(rows.smoker[1]-1, rows.drinker[1]-1, rows.active[1]-1), pos = 4, cex = cex.value, "Overall")

# add overall sample sizes for each set of analyses
text(x = pos.cov1[1], y = rows.smoker[1]-1, cex = cex.value, printnum(sum(smoker.cov1.data$n)))
text(x = pos.cov1[1], y = rows.drinker[1]-1, cex = cex.value, printnum(sum(drinker.cov1.data$n)))
text(x = pos.cov1[1], y = rows.active[1]-1, cex = cex.value, printnum(sum(active.cov1.data$n)))


# column labels
text(lower.cov1, nrow(data_cov1_all) + extra.space*2 + 1,
     "Study", cex = cex.value, pos = 4)
text(upper.cov1, nrow(data_cov1_all) + extra.space*2 + 1,
     "Interaction Coefficient Odds Ratio [95% CI]", cex = cex.value, pos=2)
text(pos.cov1[1], nrow(data_cov1_all) + extra.space*2 + 1,
     "Sample Size", cex = cex.value)
text(pos.cov1[2], nrow(data_cov1_all) + extra.space*2 + 1,
     "-1SD Con", cex = cex.value)
text(pos.cov1[3], nrow(data_cov1_all) + extra.space*2 + 1,
     "+1SD Con", cex = cex.value)
text((pos.cov1[2]+pos.cov1[3])/2, nrow(data_cov1_all) + extra.space*2 + 2,
...
# outcome labels
text(lower.cov1, rows.smoker[2] + 1,
     "Smoking Status", cex = cex.value, pos = 4)
text(lower.cov1, rows.drinker[2] + 1,
     "Alcohol Consumption", cex = cex.value, pos = 4)
text(lower.cov1, rows.active[2] + 1,
