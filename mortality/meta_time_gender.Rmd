---
title: "Neuroticism by conscientiousness by gender (time since baseline as metric)"
output: 
  html_document:
    toc: TRUE
    toc_depth: 2
...
meta.data.time.sex$study = gsub("LBC", "LBC1936", meta.data.time.sex$study)

forest(meta.results.time.sex,
       xlim = c(lower.mod1, upper.mod1),
       cex = cex.set,
       slab = meta.data.time.sex$study,
       transf=exp,
       refline = 1,
       ilab = meta.data.time.sex[,c("n",
                                "lowC.male.CI",
                                "hghC.male.CI",
                                "lowC.female.CI",
                                "hghC.female.CI")],
...
text(lower.mod1, -2, pos=4, cex=cex.set,
     bquote(paste("(Q = ",
     .(formatC(meta.results.time.sex$QE, digits=2, format="f")),
     ", df = ", .(meta.results.time.sex$k - meta.results.time.sex$p),
     ", p = ", .(formatC(meta.results.time.sex$QEp, digits=2, format="f")),
     "; ", I^2, " = ",
     .(formatC(meta.results.time.sex$I2, digits=1, format="f")), "%)")))
